...
void Llista::eliminaparells_fantasma(){
    ...
    while (act != NULL){                    // Podemos analizar todos los elementos de golpe
            if(act->elem %2 == 0){          // Si es par...
                ant->seg = act->seg;
                node *aux = act;
                delete(aux);
 ...
 // By Jordi - a) Amb fantasma
/*void Llista::eliminaparells_fantasma(){
    node *ant = _head, *act = _head->seg;
    while (ant->seg != NULL){       // o act->seg != NULL (Es el mateix)
        if(act->elem %2 == 0){      // Parell
            ant->seg = act->seg;
            delete(act);
        }
        else {
 ...
 int main(){
     ...
     while (getline(cin,line)){
         ...
         cout << "Mètodes elimina parells amb element fantasma: " << endl;
         l2.mostra();
         cout << endl;
     }
 }